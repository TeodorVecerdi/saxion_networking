...
namespace shared.net {
    ...
    public class TcpMessageChannel {
        ...
        public TcpMessageChannel(TcpClient tcpClient) {
            Log.Info($"TCPMessageChannel created around {tcpClient}", this);
        ...
        public TcpMessageChannel() {
            Log.Info("TCPMessageChannel created (not connected).", this);
        }
        ...
        public void SendMessage(object message, SerializeMode serializeMode = SerializeMode.Default) {
            if (HasErrors()) {
                Log.Error("This channel has errors, cannot send.", this);
                return;
            }

            Log.Message(message, this, ConsoleColor.Cyan);
            try {
        ...
        public object ReceiveMessage() {
            if (HasErrors()) {
                Log.Error("This channel has errors, cannot receive.", this);
                return null;
            }

            try {
                Log.Warn("Receiving message...", this, "RECV");

                var inBytes = StreamUtil.Read(stream);
                var message = Serializer.Deserialize(inBytes);
                Log.Warn($"Received {message}", this, "RECV");
        ...
        private void AddError(Exception error) {
            Log.Error($"Error added:{error}", this);
            errors.Add(error);
            Close();
        }
